...
DEFAULUT_IS_USE_LOGURU_STREAM_HANDLER = (
    True  # 是否默认使用 loguru的控制台日志，而非是nb_log的ColorHandler
)
...
LOG_FILE_BACKUP_COUNT = 1  # 对同一个日志文件，默认最多备份几个文件，超过就删除了。


current_dir: str = __file__.strip(path.basename(__file__))  # 当前文件所在的绝对路径
LOG_PATH = sep.join(
    [current_dir, "storage", "logs", "funboost"]
)  # "/fastapi-skeleton-template/storage/logs/funboost"
...
# LOG_LEVEL_FILTER = logging.CRITICAL
...
FILTER_WORDS_PRINT = [
    ...
    "推送了 1 条消息,累计推送了",
    "取出的消息是",
]  # 例如， 你希望消息中包括阿弥陀佛 或者 包括善哉善哉 就不打印，那么可以设置  FILTER_WORDS_PRINT = ['阿弥陀佛','善哉善哉']
